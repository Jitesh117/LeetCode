class Solution {
public:
    int solve(vector<int>& piles, vector<vector<int>>& dp, const vector<int>& suffixSum, int i, int M) {
        if (i == piles.size()) return 0; 
        if (i + 2 * M >= piles.size()) return suffixSum[i]; 
        if (dp[i][M] != 0) return dp[i][M];
        int result = 0;
        for (int x = 1; x <= 2 * M; ++x) {
            result = max(result, suffixSum[i] - solve(piles, dp, suffixSum, i + x, max(M, x)));
        }
        dp[i][M] = result;
        return result;
    }
